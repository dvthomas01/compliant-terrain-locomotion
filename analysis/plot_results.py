...
df = pd.read_csv("evaluation/ablation_results.csv")
...
ax1.plot(x, B.fall_rate, "s-", color="#2980b9", label="B (compliance-trained)", lw=2)
...
ax2.errorbar(x, B.forward_velocity_mean, yerr=B.forward_velocity_std, fmt="s-", color="#2980b9", label="B (compliance)", lw=2, capsize=3)
...
fig.suptitle("Compliance-randomization ablation: rigid (A) vs compliance-trained (B)", fontweight="bold")
fig.tight_layout()
out = "analysis/ablation_plot.png"; fig.savefig(out, dpi=130, bbox_inches="tight")
print("saved", out)
