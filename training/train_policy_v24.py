...
def train(cfg, run_name="policy_a_v24", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    print(f"\nPolicy A v24 — ROBUSTNESS SPRINT iter 6 (close last gate: TG lift −0.55→−0.62) — {cfg.total_timesteps:,} steps")
    print(f"  New in v24:   TG lift −0.55→{B._TG_LIFT_AMP} for ~50%% duty (single variable)")
    print(f"  Held: v23 recipe (roll W={B._W_ROLL}, yaw-rate W={B._W_YAW_RATE}, base-height LINEAR W={B._W_BASE_HEIGHT}, ACTION_SCALE 0.25)")
    print(f"  Watch: feet_in_contact→[2.0,2.5]; base_height STAY <0.30; bounce small; roll/yaw/vy/ep_len STAY in gate\n")
