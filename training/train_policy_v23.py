...
V14_LOG_STD     = -1.5
V14_TRACK_FRAC  = 0.3
V23_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v23", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V23_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v23 — ROBUSTNESS SPRINT iter 5 (tighten roll under gate: W_ROLL 70→100) — {cfg.total_timesteps:,} steps")
    print(f"  New in v23:   roll penalty W 70→{B._W_ROLL} (single variable)")
    print(f"  Held: v22 recipe (yaw-rate W={B._W_YAW_RATE}, base-height LINEAR W={B._W_BASE_HEIGHT}, lift −0.55, ACTION_SCALE 0.25)")
    print(f"  Watch: roll→<0.08; yaw_vel/vy/height STAY in gate; ep_len>800; feet_in_contact (2.87) = v24 target\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v23_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v23")
