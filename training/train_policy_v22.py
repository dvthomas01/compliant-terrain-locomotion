...
V14_LOG_STD     = -1.5
V14_TRACK_FRAC  = 0.3
V22_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v22", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V22_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v22 — ROBUSTNESS SPRINT iter 4 (pin the heading: dense yaw-rate penalty) — {cfg.total_timesteps:,} steps")
    print(f"  New in v22:   +dense quadratic yaw-rate penalty W={B._W_YAW_RATE} (single lever; no obs change)")
    print(f"  Held: v21 recipe (roll W={B._W_ROLL}, base-height LINEAR W={B._W_BASE_HEIGHT}, overspeed W=5, lift −0.55, ACTION_SCALE 0.25)")
    print(f"  Watch: yaw_vel→~0 (both signs); roll/height/vy STAY in gate; ep_len>800; feet_in_contact = next target\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v22_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v22")
