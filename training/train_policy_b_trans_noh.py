...
from environments.transition_env import TransitionEnv
...
def make_env(rank, seed=0):
    def _init():
        env = TransitionEnv(target_lin_vel=(_CMD_START, 0.0), target_ang_vel=_CMD_ANG_VEL, use_tg=True, max_level=_MAX_LEVEL, obs_mode="A")
...
def train(cfg, run_name="policy_b_trans_noh", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    def make_render_env():
        return TransitionEnv(render_mode="rgb_array", target_lin_vel=(command_curr.cmd, 0.0),
                                   target_ang_vel=_CMD_ANG_VEL, use_tg=True, max_level=_MAX_LEVEL, obs_mode="A")
    ...
    print(f"\nPolicy B_trans_noh (49D) - TRANSITION terrain-prime OBS-ABLATION - compliance terrain, 49D obs (NO history), max_level={_MAX_LEVEL} — {cfg.total_timesteps:,} steps")
    print("  Identical to Policy B EXCEPT obs_mode=A (49D, no foot history) - isolates the history contribution")
    ...
    model.save(os.path.join(checkpoint_dir, "policy_b_trans_noh_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_b_trans_noh")
