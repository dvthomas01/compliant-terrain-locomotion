...
V7_K_MAX       = 0.3
V9_ENT_COEF    = 0.0
V12_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v12", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    command_curr = CommandCurriculum(start=_CMD_START, target=_CMD_TARGET, step=0.02,
                                     advance=400.0, retreat=150.0, track_frac=0.6)  # v12: advance only when actually moving at command
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V12_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v12 — make it WALK (curriculum gated on tracking) — {cfg.total_timesteps:,} steps")
    print(f"  New in v12:   command advances only if ep_len>=400 AND vel_x>=0.6*command (was survival-only)")
    print(f"  Held from v11: log_std_init=-1.5, ent_coef=0.0, feet-air gated, overspeed, k_max=0.3")
    print(f"  Watch: command_vel rises only as vel_x keeps up; vel_x should TRACK command toward 0.2\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v12_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v12")
