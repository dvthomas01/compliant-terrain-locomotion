"""
Train Policy A v16: free the trot by halving residual authority (ACTION_SCALE 0.5→0.25).

v15 (dense roll penalty) was a BREAKTHROUGH: roll −0.27→−0.023 killed the tilt-brace, the robot
adopted a forward lean (pitch −0.217) and WALKS FORWARD for the first time — base_lin_vel_x
−0.009→+0.104, command curriculum reached the full 0.2, ep_len 858. Gate still 2/4 (ep_len ✓,
term_pitch ✓; vel_x 0.104 below [0.15,0.25] ✗, feet_in_contact 3.12 ✗).

Remaining blocker: feet_in_contact 3.12 — a real diagonal trot keeps ~2 feet down; 3.12 means the
policy still SUPPRESSES the TG foot-lift (holds ~3 feet planted), which caps stride length and
speed at 0.104. With the ±0.5 rad residual it can fully cancel the TG (lift amp 0.35, stride 0.20).

v16 makes ONE change (base_env): `ACTION_SCALE 0.5 → 0.25`. The PMTG trot supplies the gait, so the
policy should only make SMALL corrections; at 0.25 the residual is below the TG lift amplitude (0.35)
and so can no longer fully suppress the foot-lift → the propulsive full-amplitude gait dominates →
cleaner 2-foot trot → faster. Lower the suppression, raise the speed.

Held from v15: dense roll penalty W=25, PMTG TG (use_tg=True, 49D), ent_coef=0.0, command curriculum
0→0.2 from standing (track_frac=0.3), k_max=0.3, tracking σ²=0.04, pitch W=25, feet-air gated,
overspeed W=3, fwd-progress W=8, termination −20, log_std_init=−1.5.

Watch: feet_in_contact → ~2 (was 3.12); base_lin_vel_x → 0.15+ (was 0.104); ep_len should stay >800
(if it collapses <400, residual authority was load-bearing for balance → next: asymmetric scale,
full hip / reduced thigh-calf).

Usage:
    python training/train_policy_v16.py
...
V14_LOG_STD     = -1.5
V14_TRACK_FRAC  = 0.3
V16_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v16", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V16_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v16 — free the trot: ACTION_SCALE 0.5→{B.ACTION_SCALE} — {cfg.total_timesteps:,} steps")
    print(f"  New in v16:   residual ACTION_SCALE halved to {B.ACTION_SCALE} (below TG lift 0.35 → can't suppress the gait)")
    print(f"  Held: roll penalty W=25, PMTG TG (49D), ent_coef=0.0, command curriculum 0→0.2, k_max=0.3, log_std_init={V14_LOG_STD}")
    print(f"  Watch: feet_in_contact → ~2 (was 3.12); base_lin_vel_x → 0.15+ (was 0.104); ep_len stay >800\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v16_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v16")
