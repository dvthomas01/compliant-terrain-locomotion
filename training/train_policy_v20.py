...
V14_LOG_STD     = -1.5
V14_TRACK_FRAC  = 0.3
V20_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v20", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V20_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v20 — ROBUSTNESS SPRINT iter 2 (kill the banked roll → veer) — {cfg.total_timesteps:,} steps")
    print(f"  New in v20:   roll penalty W {25}→{B._W_ROLL} (single variable)")
    print(f"  Held: v19 recipe (base_height W=6, overspeed W=5, lateral W=8, roll-free-zone {B._ROLL_FREE_ZONE}, lift −0.55, ACTION_SCALE 0.25)")
    print(f"  Watch: roll→0 AND yaw_vel→0 (coupling test), ep_len>800, vel_x ~0.2, feet_in_contact not worsening\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v20_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v20")
