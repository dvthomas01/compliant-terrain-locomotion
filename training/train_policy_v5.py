"""
Train Policy A v5: backward-fix run.

Builds on v4 (which achieved real locomotion but running backward) with two
targeted changes:

  1. Backward velocity penalty (-2.0 * max(0, -lin_vel_x) * dt) outside k-curriculum.
     Makes running backward costly regardless of how stable it feels. Zero cost when
     moving forward or standing still — does not interfere with forward locomotion.

  2. Softer stability gate: exp(-2*(p²+r²)) instead of exp(-5*(p²+r²)).
     A normal 10° forward lean now retains 94% of the velocity reward vs 86% before.
     The gate still cuts reward during aggressive nose-dives; it just no longer
     discourages the mild forward lean that comes with a healthy walking gait.

Everything else is unchanged from v4:
  VecNormalize, decay=0.999, base_height w=2.0, feet_air=5.0,
  ang_vel_xy outside k at 0.15, orientation penalty, termination=-20, flat ground.

Usage:
    python training/train_policy_v5.py

    # Quick smoke-test
    python training/train_policy_v5.py --n-envs 8 --total-steps 6144

    # Monitor live
    tensorboard --logdir runs/

Success criteria (abort at 6M if none visible):
  3M  — base_lin_vel_x positive; r_backward_vel near zero
  6M  — ep_len_mean above 35 steps; robot visibly moving forward
  10M — sustained forward locomotion without nose-diving
  15M — recognisable forward walking or trotting gait
"""
...
V5_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg: PPOConfig, run_name: str = "policy_a_v5",
    ...
    print(f"\nPolicy A v5 — backward fix — {cfg.total_timesteps:,} steps")
    print(f"  Command:         {_CMD_LIN_VEL[0]} m/s forward (fixed)")
    print(f"  New in v5:       backward penalty (-2.0·|lin_vel_x|·dt when negative)")
    print(f"  New in v5:       stability gate softened exp(-5) → exp(-2)")
    print(f"  Watch for:       locomotion/base_lin_vel_x → positive")
    print(f"                   rewards/r_backward_vel → near zero")
    ...
    final_model   = os.path.join(checkpoint_dir, "policy_v5_final")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Policy A v5: backward-fix run")
    parser.add_argument("--n-envs",      type=int,  default=128)
    parser.add_argument("--total-steps", type=int,  default=15_000_000)
    parser.add_argument("--run-name",    type=str,  default="policy_a_v5")
