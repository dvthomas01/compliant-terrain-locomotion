...
V14_LOG_STD     = -1.5
V14_TRACK_FRAC  = 0.3
BNOH_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def make_env(rank, seed=0):
    def _init():
        env = CompliantTerrainEnv(target_lin_vel=(_CMD_START, 0.0), target_ang_vel=_CMD_ANG_VEL, use_tg=True, max_level=_MAX_LEVEL, obs_mode="A")
...
def train(cfg, run_name="policy_b_noh", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=BNOH_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy B-prime OBS-ABLATION - compliance terrain, 49D obs (NO history), max_level={_MAX_LEVEL} — {cfg.total_timesteps:,} steps")
    ...
    model.save(os.path.join(checkpoint_dir, "policy_b_noh_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_b_noh")
