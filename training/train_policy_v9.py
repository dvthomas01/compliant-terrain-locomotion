"""
Train Policy A v9: convergence run (kill the entropy/std explosion).

v8 proved the recurring blocker is NOT the velocity reward. With the command curriculum
starting at 0 (just stand still), the robot still couldn't survive past ~43 steps, the
deterministic policy was ~35 and DECLINING, and act_std exploded 1.0→2.1 while entropy
GREW (−17→−24.7) — the same non-convergence seen in v6/v7. The actor's policy entropy
inflates faster than the exploit gradient shrinks it, so it emits near-random extreme
joint commands and topples (now by rolling). explained_variance stayed 0.98 (critic fine).

v9 makes ONE change: ent_coef 0.01 → 0.0, to stop the entropy bonus from inflating std.
At command 0 the policy only needs to converge to standing (no exploration needed); the
command curriculum then advances in small perturbations. Everything else is held from v8.

DIAGNOSTIC: if act_std now falls and the robot learns to stand (ep_len climbs, command_vel
starts advancing), entropy was the cause. If it STILL can't stand with controlled entropy,
the next lever is control gains (Kp=20/Kd=0.5) or action scale (±0.5 rad) → v10.

Usage:
    python training/train_policy_v9.py
    tensorboard --logdir runs/

Watch:
  train/std            — MUST stop climbing / fall (the v6–v8 failure was std→2.0)
  rollout/ep_len_mean  — should rise toward horizon at command 0
  train/command_vel    — should begin advancing once ep_len clears 400
"""
...
V7_K_MAX      = 0.3
V9_ENT_COEF   = 0.0    # was 0.01 — the entropy bonus was inflating action std to ~2.0 (non-convergence)
V9_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v9", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    command_curr = CommandCurriculum(start=_CMD_START, target=_CMD_TARGET, step=0.02, advance=400.0, retreat=150.0)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V9_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v9 — convergence (ent_coef {0.01}→{V9_ENT_COEF}) — {cfg.total_timesteps:,} steps")
    print(f"  Watch: train/std must STOP climbing; ep_len rise at command 0; command_vel advance\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v9_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v9")
