...
V14_LOG_STD     = -1.5
V14_TRACK_FRAC  = 0.3
V21_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v21", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V21_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v21 — ROBUSTNESS SPRINT iter 3 (fix the crouch: linear base-height penalty) — {cfg.total_timesteps:,} steps")
    print(f"  New in v21:   base-height penalty quadratic→one-sided LINEAR, W={B._W_BASE_HEIGHT} (single lever)")
    print(f"  Held: v20 recipe (roll W={B._W_ROLL}, overspeed W=5, lateral W=8, roll-free-zone {B._ROLL_FREE_ZONE}, lift −0.55, ACTION_SCALE 0.25)")
    print(f"  Watch: base_height→~0.26; roll/yaw_vel must STAY solved; feet_in_contact must not spike; ep_len>800\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v21_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v21")
