...
V7_K_MAX       = 0.3
V9_ENT_COEF    = 0.0
V10_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v10", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V10_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v10 — stable standing (feet-air gated by command) — {cfg.total_timesteps:,} steps")
    print(f"  New in v10:   r_feet_air scaled by min(1,|cmd|/{B._FEET_AIR_REF_SPEED}) (0 at command 0)")
    print(f"  Held from v9: ent_coef=0.0, command curriculum 0→0.2, overspeed, penalty cap k_max=0.3")
    print(f"  Watch: at cmd 0 feet_in_contact→~4, ep_len→horizon; then command_vel advances\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v10_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v10")
