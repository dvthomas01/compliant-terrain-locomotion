...
V7_K_MAX       = 0.3
V9_ENT_COEF    = 0.0
V11_CHECKPOINTS = [1_000_000, 3_000_000, 6_000_000, 10_000_000, 15_000_000]
...
def train(cfg, run_name="policy_a_v11", render_freq=500_000, live_viewer=False):
    preflight(cfg.gamma)
    ...
    model = PPO(
        ...
        policy_kwargs=dict(net_arch=dict(pi=[256, 128], vf=[256, 128]), activation_fn=nn.ELU,
                           log_std_init=-1.5),  # v11: initial action std exp(-1.5)=0.22 (was 1.0) → start near the stable stand
    ...
    callbacks = [
        ...
        VecNormCheckpointCallback(vec_env=vec_env, steps=V11_CHECKPOINTS, save_dir=checkpoint_dir, prefix="model"),
    ]
    if render_freq > 0:
    ...
    print(f"\nPolicy A v11 — find the stand (log_std_init=-1.5) — {cfg.total_timesteps:,} steps")
    print(f"  New in v11:   initial action std 1.0→0.22 so the policy starts near the stable nominal stand")
    print(f"  Held from v10: ent_coef=0.0, feet-air gated by cmd, command curriculum 0→0.2, overspeed, k_max=0.3")
    print(f"  Watch: ep_len should FINALLY climb toward horizon at command 0; then command_vel advances\n")
    model.learn(total_timesteps=cfg.total_timesteps, callback=CallbackList(callbacks), progress_bar=True)

    model.save(os.path.join(checkpoint_dir, "policy_v11_final"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--run-name", type=str, default="policy_a_v11")
